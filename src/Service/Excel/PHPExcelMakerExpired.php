...
class PHPExcelMakerExpired extends ExcelMaker
{
    public function run() {
        ...
        $objPHPExcel->getActiveSheet()
            ->setCellValue('A' . $line++, 'Название отчета: Отчет по лидам/сделкам с просроченными действиями')
        ...
        if ($this->config['total_leads']) {
            $procent_leads = round(((count($this->config['expired_activity_leads']) * 100) / count($this->config['total_leads'])), 2);
        } else {
            $procent_leads = 0;
        }

        if ($this->config['total_deals']) {
            $procent_deals = round(((count($this->config['expired_activity_deals']) * 100) / count($this->config['total_deals'])), 2);
        } else {
            $procent_deals = 0;
        }
        ...
        $objPHPExcel->getActiveSheet()
            ->setCellValue('A' . $line, 'Итого (с просроченными действиями)')
            ->setCellValue('B' . $line, count($this->config['expired_activity_leads']))
            ->setCellValue('C' . $line, count($this->config['expired_activity_deals']));
        ...
        $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('B' . $line, 'Лиды (с просроченными действ.)')
            ->setCellValue('C' . $line, 'Всего лидов за день')
            ->setCellValue('D' . $line, 'Сделки (с просроченными действ.)')
        ...
        $leads_and_deals = array_merge($this->config['expired_activity_leads'], $this->config['expired_activity_deals']);
